...
function runGitPublish(siteDir, message) {
...
function publishBeta() {
  return runGitPublish(WEBSITE_DIR, 'Publish to Beta');
}

async function publishLive() {
  try {
    if (!fs.existsSync(LIVE_SITE_DIR)) {
      return {
        ok: false,
        error: `Live site folder not found: ${LIVE_SITE_DIR}`
      };
    }

    fs.copyFileSync(INDEX_PATH, LIVE_INDEX_PATH);

    const betaImagesDir = path.join(WEBSITE_DIR, 'images');
    const liveImagesDir = path.join(LIVE_SITE_DIR, 'images');

    if (fs.existsSync(betaImagesDir)) {
      if (!fs.existsSync(liveImagesDir)) {
        fs.mkdirSync(liveImagesDir, { recursive: true });
      }

      fs.cpSync(betaImagesDir, liveImagesDir, {
        recursive: true
      });
    }

    return await runGitPublish(LIVE_SITE_DIR, 'Publish to Live');
  } catch (error) {
    return {
      ok: false,
      error: error.message
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'POST' && req.url === '/api/preview-save') {
      const data = JSON.parse(await readBody(req));
      const result = updateHtml(data);
      send(res, 200, result);
      return;
    }

    if (req.method === 'POST' && req.url === '/api/publish-beta') {
      const result = await publishBeta();
      send(res, result.ok ? 200 : 500, result);
      return;
    }

    if (req.method === 'POST' && req.url === '/api/publish-live') {
